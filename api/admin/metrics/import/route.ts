...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = parseMetricsCsv(data.csv);
  const validation = validateRows(parsed, metricValidators);
  ...
  const result = await persistImport({ entity: "metrics", rows: validation.accepted }, defaultPersistAdapter);
